...
- `interview-kubernetes` — DevOps interview cheat sheet for Kubernetes covering architecture (control plane, nodes), core concepts (Pods, Deployments, Services, Ingress), configuration (ConfigMaps, Secrets), networking, storage, scheduling, autoscaling, RBAC, and common interview questions with concise answers. (`~/.globalskills/skills/interview-kubernetes/SKILL.md`).
