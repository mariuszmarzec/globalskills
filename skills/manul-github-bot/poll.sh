...
for repo in "${REPOS[@]}"; do
  ...
  # Skip review comments in resolved review threads to avoid re-triggering
  # manul from old/historical threads that are no longer actionable.
  UNRESOLVED_TMP="$(mktemp)"
  unresolved_prs=()
  while read -r pr_number; do
    unresolved_prs+=("$pr_number")
  done < <(gh api --paginate "repos/$repo/pulls?state=open&per_page=100" --jq '.[].number' 2>>"$LOG")

  declare -A UNRESOLVED_COMMENT_IDS
  for pr_number in "${unresolved_prs[@]}"; do
    mapfile -t thread_ids < <(gh api --paginate "repos/$repo/pulls/$pr_number/comments?per_page=100" 2>>"$LOG" | jq -r '.[] | select(.in_reply_to_id == null) | .id' 2>/dev/null || true)
    for tid in "${thread_ids[@]}"; do
      [ -n "$tid" ] || continue
      UNRESOLVED_COMMENT_IDS["$tid"]=1
    done
  done

  is_unresolved() {
    local cid="$1"
    if [ -n "${UNRESOLVED_COMMENT_IDS[$cid]:-}" ]; then
      return 0
    fi
    return 1
  }
  ...
  while IFS= read -r obj; do
    [ -n "$obj" ] || continue
    id="$(jq -r '.id' <<<"$obj")"
    [ -n "${UNRESOLVED_COMMENT_IDS[$id]:-}" ] || continue
    issue="$(jq -r '.issueNumber' <<<"$obj")"
    url="$(jq -r '.url' <<<"$obj")"
    author="$(jq -r '.author' <<<"$obj")"
    created="$(jq -r '.created' <<<"$obj")"
