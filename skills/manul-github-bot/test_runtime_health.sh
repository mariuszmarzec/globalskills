...
[ -f "$1/state/manul.db" ] && cp "$1/state/manul.db" "$CANON/" 2>/dev/null || true
...
cp "$CLI_DB" "$1/state/manul.db"
chmod 600 "$1/state/manul.db"
...
for table in $REQUIRED_TABLES; do
  if ! sqlite3 "$1/state/manul.db" "SELECT 1 FROM $table LIMIT 1;" >/dev/null 2>&1; then
...
DB_BEFORE=$(sha256sum "$1/state/manul.db" | awk '{print $1}')
...
DB_AFTER=$(sha256sum "$1/state/manul.db" | awk '{print $1}')
...
if [ -f "$1/state/manul.db" ]; then
  if sqlite3 "$1/state/manul.db" "SELECT COUNT(*) FROM processed_comments;" >/dev/null 2>&1; then
...
WS_INIT_OUTPUT=""
WS_INIT_EXIT=0
WS_INIT_OUTPUT="$(MANUL_DIR="$WSFAIL_DIR" DB="$1/state/manul.db" \
...
touch "$1/state/manul.db"
chmod 644 "$1/state/manul.db"
...
echo "not a database" > "$1/state/manul.db"
chmod 644 "$1/state/manul.db"
...
if [ -f "$1/state/manul.db" ]; then
  if sqlite3 "$1/state/manul.db" "SELECT 1 FROM processed_comments LIMIT 1;" 2>/dev/null | grep -q .; then
...
sqlite3 "$1/state/manul.db" "INSERT OR REPLACE INTO meta(key,value) VALUES('baseline','2019-01-01T00:00:00Z');"
...
for tbl in processed_comments conversations meta workspaces; do
  if ! sqlite3 "$1/state/manul.db" "SELECT 1 FROM $tbl LIMIT 1;" >/dev/null 2>&1; then
...
if [ -f "$1/state/manul.db" ] && [ -f "$INSTALL_ROOT/config.json" ]; then
  ok "Fresh installer creates DB and config"
else
  fail "Fresh installer did not create DB/config"
fi

INSTALL_BASELINE_VALUE="$(sqlite3 "$1/state/manul.db" "SELECT value FROM meta WHERE key='baseline';" 2>/dev/null || true)"
...
if [ "$INSTALL_EXIT_2" -eq 0 ]; then
  INSTALL_BASELINE_AFTER="$(sqlite3 "$1/state/manul.db" "SELECT value FROM meta WHERE key='baseline';" 2>/dev/null || true)"
...
INIT_EXIT=$?
set -e
if [ "$INIT_EXIT" -eq 0 ] && [ -s "$1/state/manul.db" ]; then
